const covidGame = {
  ...
  createPlayer() {
    this.player = new Player(
      this.ctx,
      this.canvasSize,
      this.canvasSize.width / 2 - 25,
      this.canvasSize.height - 170,
      50,
      165,
      "jeringa.png"
    );
  ...
  createDirectionChangedImg() {
    this.directionChangedImg = new ChangeDirection(
      this.ctx,
      this.canvasSize.width / 2 - 150,
  ...
  createObstacles() {
    ...
    this.obstacles.push(
      new Obstacle(this.ctx, randomX, 50, 70, 70, randomSpeed, "bacteria.png")
    );
  },
  ...
  createExtralife() {
    ...
    this.extraLife.push(
      new ExtraLife(this.ctx, randomX, 50, 120, 70, randomSpeed, "mask.png")
    );
  },
  ...
  detectCollision() {
    return this.player.bullets.some((bullet, bulletIndex) => {
      return this.obstacles.some((obstacle, obstacleIndex) => {
        if (
          bullet.pos.x < obstacle.posX + obstacle.width &&
  ...
  detectExtraLife() {
    return this.player.bullets.some((bullet, bulletIndex) => {
      return this.extraLife.some((exLife, exLifeIndex) => {
        if (
          bullet.pos.x < exLife.posX + exLife.width &&
...
window.addEventListener("DOMContentLoaded", function () {
  if (localStorage.getItem("backgroundMusic") === "play") {
